...
We then computed the Variance Inflation Factor (VIF), which assesses whether a variable exhibits excessive correlation with other predictors, rendering it redundant in the model. In our analysis, log(tenure + 0.01), MonthlyCharges, log(TotalCharges + 0.01), InternetService, and MultipleLines demonstrated relatively high VIF values, indicating a notable level of multicollinearity among these variables. Consequently, we opted to exclude MonthlyCharges and log(TotalCharges + 0.01) from the model due to their significant correlation with other variables. These results lead to a loss of deviance, but there is no apparent collinearity in our model.
...
m3f <- glm(formula = Churn ~ log(tenure + 0.01) + Contract + OnlineSecurity + TechSupport + InternetService + 
    PaymentMethod + MultipleLines + PaperlessBilling + SeniorCitizen + StreamingMovies + StreamingTV, family = binomial, data = customer_churn)
...
In the influence plot, residuals still range from -2 to 3. However, the Cook's distance has decreased, indicating that the overall fit of the model is more robust, and individual observations have a lesser impact on the results. However, some observations with medium leverage persist. Nevertheless, these observations demonstrate a low to medium Cook's distance, indicating that they do not exert a substantial influence on the fitted values or estimated coefficients.

```{r}
residualPlots(m3f, layout =c(1,2))
...
We started by introducing an interaction between log(tenure + 0.01) and Contract. The comparison between the models (m3f and m3i) revealed a statistically significant improvement, indicating that the interaction term enhances the model's fit. Next, we introduced an interaction between SeniorCitizen and PaymentMethod. Similar to the previous case, the comparison between models (m3i and m4i) showed a significant improvement, suggesting that the interaction term contributes significantly to the model. Finally, we explored the interaction between InternetService and MultipleLines. However, the comparison between models (m4i and m5i) did not yield statistical significance, indicating that this particular interaction did not significantly enhance the model. We further compared the models using the AIC. The model with the lowest AIC, in this case, m4i, is considered the best among the tested models.

```{r}
#tenure and contract
m3i <- glm(formula = Churn ~ log(tenure + 0.01) * Contract + OnlineSecurity + TechSupport + InternetService + MultipleLines 
           + PaperlessBilling + SeniorCitizen + PaymentMethod + StreamingMovies + StreamingTV, family = binomial, data = customer_churn)

anova( m3f, m3i, test="Chisq")#significant
waldtest( m3f, m3i, test="Chisq" )
...
anova( m3i, m4i, test="Chisq")#significant
waldtest( m3i, m4i, test="Chisq" )

# InternetService and MultipleLines
m5i <- glm(formula = Churn ~ log(tenure + 0.01) * Contract + OnlineSecurity + TechSupport + InternetService * MultipleLines 
           + PaperlessBilling + SeniorCitizen * PaymentMethod + StreamingMovies + StreamingTV, family = binomial, data = customer_churn)

anova( m4i, m5i, test="Chisq")#no significant
waldtest( m4i, m5i, test="Chisq" )
...
```{r}
m6i <- glm(formula = Churn ~ poly(tenure,2) + log(tenure + 0.01) * Contract + OnlineSecurity + TechSupport + InternetService + MultipleLines 
           + PaperlessBilling + SeniorCitizen * PaymentMethod + StreamingMovies + StreamingTV, family = binomial, data = customer_churn)

anova(m4i, m6i, test="Chisq")#significant
waldtest(m4i, m6i, test="Chisq")
AIC(m4i, m6i)
```
...
```{r}
marginalModelPlots(m6i,id=list(labels=row.names(customer_churn),method=abs(cooks.distance(m6i)), n=5))

residualPlots(m6i, layout=c(2, 2))

llnrem<-influencePlot(m6i,id=list(n=3) );llnrem

llistal<-Boxplot(hatvalues(m6i), id.n=10)
abline(h=4*length(coef(m6i))/nrow(customer_churn))
hatvalues(m6i)[llistal]

llistac<-Boxplot(cooks.distance(m6i), id=list(labels=rownames(customer_churn)))
cooks.distance(m6i)[llistac]
customer_churn[llistac,]
llrem <- which( rownames(customer_churn) %in% c("3972","659","5948"));llrem

customer_churn <- customer_churn[-llrem,]

final_model <- glm(formula = Churn ~ poly(tenure,2) + log(tenure + 0.01) * Contract + OnlineSecurity + TechSupport + InternetService + MultipleLines 
           + PaperlessBilling + SeniorCitizen * PaymentMethod + StreamingMovies + StreamingTV, family = binomial, data = customer_churn)
```

# Goodness of fit and Model Interpretation

The McFadden pseudo R-squared has a moderate value which suggests that the model exlains some of the variation in the response variable. However, null hypothesis is rejected due to potential collinearity problems. Based on these results, we can say that the model has a moderate predictive power, without a great goodness-of-fit.
...
The metrics show that the model correctly classified about 81% of the training data, 67% of the observations that it predicted as positive and 54% of the observations that are actually positive. The F1 score of 60% means that the model has a moderate performance in terms of both precission and recall.
