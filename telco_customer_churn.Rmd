...
The formula is modified and the product of the 3 numeric variables is added. This more complex model is significantly better as the difference in deviance is 17.745. The null hypothesis can be rejected and conclude that the two models are different. In addition, the model is more parsimonious and has a better trade-off between the fit and the complexity of the model.

```{r}
model_numeric_best <- glm(
  Churn ~ tenure
  * MonthlyCharges
...
anova(model_numeric_basic, model_numeric_best, test = "Chisq")

AIC(model_numeric_basic, model_numeric_best)
```

## Categorical variables

We first add categorical variables one by one to observe the impact of each categorical variable on the model. The variable f.tenure is excluded from consideration, as its inclusion does not result in a significantly improved fit. However, the remaining categorical variables, which exhibit a higher influence on Churn as analyzed in the Profiling segment, substantially enhance the model. This improvement is evident through a noticeable difference in deviance. In the case of the DeviceProtection variable, ANOVA indicates that the models are predicting the same outcomes, leading us to conclude our additions there. Further validation through the AIC test and step demonstrates that the m5f model is the best one.
...
anova(m1f, m2f, test = "Chisq")# No improvement
...
anova(m3f, m4f, test = "Chisq")

m5f <- glm(
  Churn ~ tenure
  * MonthlyCharges
  * log(TotalCharges + 0.01)
  + Contract + OnlineSecurity + TechSupport + InternetService + PaymentMethod + OnlineBackup,
  data = customer_churn,
  family = binomial
)

anova(m4f, m5f, test = "Chisq")

m6f <- glm(
  Churn ~ tenure
  * MonthlyCharges
  * log(TotalCharges + 0.01)
  + Contract + OnlineSecurity + TechSupport + InternetService + PaymentMethod + OnlineBackup + DeviceProtection,
  data = customer_churn,
  family = binomial
)

anova(m5f, m6f, test = "Chisq")# No improvement

AIC(m3f, m4f, m5f)
step(m5f, trace = 0)
```

Residual analysis: unusual and influent data filtering. Missing analysis

```{r}
residualPlots(m5f, layout=c(1,2))
plot(allEffects(m5f), ask = FALSE)
influencePlot(m5f)
Boxplot(cooks.distance(m5f))
```
